...
SELECT
    ...
    (SELECT COUNT(*) AS `total` FROM `source_database` AS sd2 RIGHT JOIN `taxon` AS `t` ON sd2.id = t.source_database_id LEFT JOIN `taxon_detail` AS `td` ON t.id = td.taxon_id WHERE (sd2.id = sd.id AND t.taxonomic_rank_id = 83 AND td.is_extinct = 1)) AS number_of_extinct_species,
    (SELECT COUNT(*) AS `total` FROM `source_database` AS sd3 RIGHT JOIN `taxon` AS `t` ON sd3.id = t.source_database_id  LEFT JOIN `taxon_detail` AS `td` ON t.id = td.taxon_id WHERE (sd3.id = sd.id AND t.taxonomic_rank_id NOT IN (54,76,6,72,112,17,20,96,83) AND td.is_extinct = 1)) AS number_of_extinct_infraspecific_taxon,
