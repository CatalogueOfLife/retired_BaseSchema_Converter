    SELECT
        ...
        IF (t_1.`taxonomic_rank_id` IN (54,76,6,72,17,112,20),CONCAT(UCASE(SUBSTRING(sne_1.`name_element`, 1, 1)),LOWER(SUBSTRING(sne_1.`name_element`, 2))),
            ...
            CONCAT(CONCAT(UCASE(SUBSTRING(sne_3.`name_element`, 1, 1)),LOWER(SUBSTRING(sne_3.`name_element`, 2)))," ",sne_2.`name_element`,IF(tr1.`marker_displayed` != "",CONCAT(" ",tr1.`marker`),"")," ",sne_1.`name_element`)
